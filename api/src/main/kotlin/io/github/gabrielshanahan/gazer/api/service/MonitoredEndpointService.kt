...
class MonitoredEndpointService(
    ...
    fun UserAuthentication.findAll(): List<MonitoredEndpoint> = endpointRepository
    ...
    fun UserAuthentication.findOwn(id: String): MonitoredEndpoint? = withOwn(id) { it.asModel() }

    fun UserAuthentication.findRelatedTo(id: String, limit: Int?): List<MonitoringResult>? = withOwn(id) {
        resultService.findRelatedTo(it, limit)
    }

    fun UserAuthentication.create(endpoint: MonitoredEndpoint): MonitoredEndpoint = MonitoredEndpointEntity(
        name = endpoint.name!!,
        url = endpoint.url!!,
    ...
    fun validate(endpoint: MonitoredEndpoint) {
    ...
    fun UserAuthentication.delete(id: String): Unit? = withOwn(id) {
